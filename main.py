...
def run(kwargs: dict, model_params_list: list, filename_list: list):
    ...
    if run_option == 'covert_to_pdb':
        ...
        tr = DataTrajectory(**kwargs)
        SingleTrajectoryAnalyser(tr, kwargs[PARAMS]).compare(model_params_list)
    elif run_option == COMPARE_WITH_CA_ATOMS:
        tr = DataTrajectory(**kwargs)
        SingleTrajectoryAnalyser(tr, kwargs[PARAMS]).compare_with_carbon_alpha_and_all_atoms(model_params_list)
    elif run_option == BASE_TRANSFORMATION:
        tr = DataTrajectory(**kwargs)
        SingleTrajectoryAnalyser(tr, kwargs[PARAMS]).compare_with_basis_transformation(model_params_list)
    elif run_option == PARAMETER_GRID_SEARCH:
        param_grid = config.get_param_grid()
        tr = DataTrajectory(**kwargs)
        SingleTrajectoryAnalyser(tr, kwargs[PARAMS]).grid_search(param_grid)
